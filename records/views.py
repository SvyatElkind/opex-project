...
class AddRecordAPIView(ProjectRelationMixin, ResponseMixin, APIView):
    ...
    def post(self, request, project_id):
        ...
        try:
            ...
            logger.warning(f"{self.__class__.__name__}: {ex}")
...
class AddMetadataAPIView(ProjectRelationMixin, ResponseMixin, APIView):
    ...
    def post(self, request, project_id, record_id):
        ...
        if not serializer_class:
            logger.warning(f"{self.__class__.__name__}: Unknown class {model_class_name}")
...
class MetadataAPIView(ProjectRelationMixin, ResponseMixin, APIView):
    ...
    def put(self, request, project_id, record_id):
        ...
        if not metadata_instance:
            logger.warning(f"{self.__class__.__name__}: Metadata instance not found with id {metadata_id}")
...
class MultipleFileUploadAPIView(ProjectRelationMixin, ResponseMixin, APIView):
    ...
    def post(self, request, project_id, record_id):
        ...
        if not files:
            logger.warning(f"{self.__class__.__name__}: No files provided")
...
class FileDeleteAPIView(ResponseMixin, APIView):
    ...
    def delete(self, request, project_id, file_id):
        ...
        if not file:
            logger.warning(f"{self.__class__.__name__}: File with id {file_id} not found")
            return self.response({ERROR: MSG_E_NO_FILE.format(file_id)}, 400)
        if not file.record:
            logger.warning(f"{self.__class__.__name__}: File with id {file_id} is not related to any text record")
...
class AddMediaRecordAPIView(ProjectRelationMixin, ResponseMixin, APIView):
    ...
    def post(self, request, project_id):
        ...
        item_id = request.query_params.get('item_id')
        if not item_id:
            logger.warning(f"{self.__class__.__name__}: No item_id provided")
            return self.response({ERROR: MSG_E_NO_INTEM_ID}, 400)
        
        item = Item.objects.filter(id=item_id).first()
        if not item:
            logger.warning(f"{self.__class__.__name__}: Item with id {item_id} not found")
        ...
        if not file:
            ...
            return self.response({ERROR: MSG_E_NO_FILES_PROVIDED}, 400)
        if len(file) > 1:
            logger.warning(f"{self.__class__.__name__}: Multiple files provided, only one is allowed")
...
class MediaRecordAPIView(ProjectRelationMixin, ResponseMixin, APIView):
    ...
    def put(self, request, project_id, record_id):
        ...
        if not record_type:
            logger.warning(f"{self.__class__.__name__}: No type provided")
            return self.response({ERROR: MSG_E_NO_TYPE_PROVIDED}, 400)
        
        record_class = MEDIA_CLASS_MAP.get(record_type)
        if not record_class:
            logger.warning(f"{self.__class__.__name__}: Unknown class {record_type}")
